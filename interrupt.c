...
void main()
{
    TRISB=0xF0; //initalizing switch configuration
    ...
    INTCON=0x88; //initalizing INTCON for GIE AND RBIE
...
void __interrupt() ISR() //initialzing INTERRUPT
 {
     if(INTCON & 0x01) //enabling flag on RBIF
     {
         val = PORTB;  // enable switch for PORTB
         if (val == 0xE0)  // if we press RB4
         {
             PORTC =~ PORTC; //PORTC Led on and off alternative
             __delay_ms(200);
         }
         INTCON &= 0xFE; //Turn off the flag
         __delay_ms(5000);
     }
 }